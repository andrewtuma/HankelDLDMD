...
def train_model(hyp_params, train_data, val_set, model, loss):
    ...
    max_win_stp = 1
    counter = 1
    net_obs_var = [model.num_observables * model.phys_dim]
    ...
    for epoch in range(1, hyp_params['max_epochs'] + 1):
        ...
        with tf.device(hyp_params['device']):
            ...
            print("num observables")
            print(model.num_observables * model.phys_dim)
            net_obs_var.append(model.num_observables * model.phys_dim)
